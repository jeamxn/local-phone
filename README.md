...
| 패키지 | 설명 | 포트 |
|--------|------|------|
| `apps/web` | Next.js(App Router) + antd 프런트엔드. 통화 UI 전부 | 3000 |
| `apps/signal` | `ws` 시그널링 서버 + 채팅 릴레이 + **Gemini Live 번역 릴레이** (Vertex AI SA 인증) | 8080 |
...
루트 `docker-compose.yml` 참고. Dokploy/Coolify에서 빌드되며 web(3000)/signal(8080) 두 서비스를 띄운다.
